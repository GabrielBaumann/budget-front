<?php $this->layout('layout_pagina', ['title' => $title, 'unidades' => $unidades]) ?>

<!-- Conteúdo Principal -->
<main class="flex-grow w-full max-w-6xl mx-auto px-4 sm:px-6 py-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold">Unidades de medidas</h2>
        <a href="<?= url("/uni"); ?>">
    ...
    <div class="overflow-hidden rounded-lg md:border md:border-gray-200 md:dark:border-gray-600">
        <table class="responsive-table min-w-full divide-y divide-gray-200 dark:divide-gray-600">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Unidade</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Abreviação</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                <?php foreach ($unidades as $unidade): ?>
                    <tr>
                        <td data-label="Nome" class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100"><?= $unidade->unidade?></td>
                        <td data-label="Email" class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300"><?= $unidade->abreviacao?></td>
                        <td data-label="Ações" class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                            <button class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3">
                                <a href="<?= url("/uni/{$unidade->id_unidade}"); ?>" rel="noopener noreferrer">Editar</a>
                            </button>
